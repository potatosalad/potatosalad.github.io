...
def test_theme_responsive_content_colors() -> None:
    ...
    require("$sidebar-desktop-breakpoint: 850px" in stylesheet, "collapsed rail must remain until Chirpy's hamburger breakpoint")
    require("max-width: calc($sidebar-desktop-breakpoint - 1px)" in stylesheet, "mobile widths must retain the drawer layout")
    require("#sidebar-trigger,\n  #search-trigger {\n    display: block !important" in stylesheet, "hamburger must appear whenever the responsive drawer replaces the rail")
    ...
    require("position: fixed !important" in stylesheet, "collapsed theme selector menu must escape the rail clipping boundary")
    require("overflow: visible" in stylesheet, "collapsed theme selector overlay must remain clickable outside the rail")
    require("left: calc($sidebar-rail-width + 0.5rem) !important" in stylesheet, "collapsed theme selector menu must open beside the rail")
